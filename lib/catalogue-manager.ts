...
export async function getPublicManagedTracks(releaseId:string): Promise<ManagedTrack[]> { const c=publicConfig(); if(!c) return [] as ManagedTrack[]; const response=await fetch(`${c.url}/rest/v1/penrec_tracks?select=*&release_id=eq.${encodeURIComponent(releaseId)}&order=track_number.asc`,{headers:{apikey:c.key,Authorization:`Bearer ${c.key}`},cache:"no-store"}); if(!response.ok) return [] as ManagedTrack[]; return (await response.json()) as ManagedTrack[]; }
