...
export async function upsertManagedTracks(payloads: Omit<ManagedTrack,"id"|"created_at"|"updated_at">[]) {
  if (!payloads.length) return [] as ManagedTrack[];
  const updated_at = new Date().toISOString();
  return restRequest<ManagedTrack[]>("penrec_tracks?on_conflict=release_id,track_number", {method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify(payloads.map(payload=>({...payload,updated_at})))}, await token());
}
...
async function publicBearer(){ return (await getAccessToken()) || publicConfig()?.key || null; }
export async function getPublicManagedReleases(): Promise<ManagedRelease[]> { const c=publicConfig(); if(!c) return [] as ManagedRelease[]; const bearer=await publicBearer(); if(!bearer) return [] as ManagedRelease[]; const response=await fetch(`${c.url}/rest/v1/penrec_releases?select=*,artist:penrec_artists(name,slug)&status=eq.published&order=release_date.desc.nullslast,updated_at.desc`,{headers:{apikey:c.key,Authorization:`Bearer ${bearer}`},cache:"no-store"}); if(!response.ok) return [] as ManagedRelease[]; return (await response.json()) as ManagedRelease[]; }

export async function getPublicManagedTracks(releaseId:string): Promise<ManagedTrack[]> { const c=publicConfig(); if(!c) return [] as ManagedTrack[]; const bearer=await publicBearer(); if(!bearer) return [] as ManagedTrack[]; const response=await fetch(`${c.url}/rest/v1/penrec_tracks?select=*&release_id=eq.${encodeURIComponent(releaseId)}&order=track_number.asc`,{headers:{apikey:c.key,Authorization:`Bearer ${bearer}`},cache:"no-store"}); if(!response.ok) return [] as ManagedTrack[]; return (await response.json()) as ManagedTrack[]; }
