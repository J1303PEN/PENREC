...
export async function buildFifthMainFlexfitCap() {
  ...
  const response = await fetch("https://api.printful.com/products/140", {
  ...
  const payload = await response.json() as {
    result?: {
      product?: { id?: number; title?: string; image?: string; type_name?: string };
      variants?: Array<{ id?: number; name?: string; size?: string; color?: string; price?: string; currency?: string; in_stock?: boolean }>;
    };
  };

  const parent = payload.result?.product;
  const variants = (payload.result?.variants || []).filter((variant) =>
    String(variant.color || "").toLowerCase() === "black" && ["S/M", "L/XL"].includes(String(variant.size || "")) && variant.in_stock !== false
  );

  if (!parent?.id || variants.length === 0) {
    redirect("/admin/merchandise?error=Printful+did+not+return+the+black+Flexfit+6277+variants");
  }
  ...
  for (const variant of variants) {
    ...
    await createProduct({
      ...
      title: `Fifth & Main Signature Cap — ${size}`,
      artist_slug: "fifth-and-main",
      release_id: null,
      product_type: "accessory",
      format: "Accessory",
      description: "Closed-back structured Flexfit 6277 cap in black, mapped to the live Printful catalogue for front embroidery.",
