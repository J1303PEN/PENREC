...
function albumTracks(items: [string, string][], preview: string): Track[] {
  return items.map(([title, duration], index) => ({
    title,
    duration,
    audio: index === 0 ? preview : undefined,
  }));
}
